...
<img width="1222" height="288" alt="download" src="https://github.com/user-attachments/assets/e5010eda-30cc-4e08-9258-61b60b4b9e76" />
  Now due to the volatility,  I only fit our 3 largest serve samples per match (matches 1,5,9) to the arima model using auto_arima to reduce error for each team. Below is a graph of our actual vs our model for match 9 team b.
...
# Discussion
  ...
  For our third question, we can say that jump serving is considered far more high risk high reward as when jump serving is effective it is seriously punishing for an opposing team. Float serves are far more safe and errors with float serves are much less frequent but they come with downside of being far less efficient. One application of this is if you know a team is really good at passing you will need a more risky approach of using more jump servers to keep them out of system. On the flip side if you are playing an overall weaker team that can't pass as well you can use more float servers to play safer because the payoff of jump serving is so much lower than just float serving and keeping the ball in play. 

  Some ideas for future research would be to develop a more streamlined and nuanced metric for efficiency that rewards good serves that aren't aces. A 1 pass is a very difficult pass to win a point out of so I feel that metric could pretty represent the true efficiency. Other ideas include get a bigger sample size or using both teams data for modeling so we get more samples and still can predict the tit for tat overall percentage change. 
